...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "SELECT  `admin_Password` FROM `admin_register` WHERE `admin_UserName`= '$for_doc_session' ";
    ...
    if(mysqli_num_rows($result) > 0) {
        ...
        if ($hashed_password === $passwordFromDB) {
            ...
            if ($newPassword === $confirmNewPassword) {
                // $sql2 = "SELECT  `admin_Password`, `admin_Confirmpassword`, FROM `admin_register` WHERE `admin_UserName`= '';
